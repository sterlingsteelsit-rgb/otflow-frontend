...
// ─── module-level pure helper — not recreated on every render ────────────────
...
// Stable outside component so shiftOptions useMemo can use [] dep safely
const SHIFT_OPTIONS = SHIFTS.map((s) => ({ label: s.label, value: s.value }));
...
export function OtEntryPage() {
  ...
  // Pre-compute string keys once per week change — used in render + async fns
  const weekDateKeys = useMemo(() => weekDates.map(toYYYYMMDD), [weekDates]);

  const weekRange = useMemo(
    () => ({ from: weekDateKeys[0], to: weekDateKeys[6] }),
    [weekDateKeys],
  );
  ...
  // O(1) lookup — replaces every selectedIds.includes() call
  ...
  // Reasons cache — avoids repeat API calls when reopening modals
  const reasonsCache = useRef<{ APPROVE?: ReasonOpt[]; REJECT?: ReasonOpt[] }>(
    {},
  );

  // AbortController as ref — avoids a state render on every date switch
  const dayReqRef = useRef<AbortController | null>(null);
  ...
  // ─── Pending IDs memoized — drives header checkbox and toggleSelectAll ───
  const approvablePendingIds = useMemo(
    () => dayItems.filter((x) => x.status === "PENDING").map((x) => x._id),
    [dayItems],
  );

  // ─── Selected rows derived from Set — O(n) not O(n²) ────────────────────
  const selectedRows = useMemo(
    () => dayItems.filter((x) => selectedSet.has(x._id)),
    [dayItems, selectedSet],
  );
  ...
  async function loadWeekCompleteness(signal?: AbortSignal) {
    ...
    try {
      ...
      // weekDateKeys is memoized at component level — no inline .map() needed
      for (const d of weekDateKeys) {
  ...
  async function loadDay(date: string) {
    // Cancel previous in-flight request via ref — no state render cost
    dayReqRef.current?.abort();
    ...
    try {
      const r = await api.get("/ot", {
        params: { from: date, to: date, page: 1, limit: 200 },
        signal: c.signal,
      });
  ...
  // ─── Reasons — cached so modals don't re-fetch on every open ────────────
  const fetchReasons = useCallback(async (type: "APPROVE" | "REJECT") => {
    if (reasonsCache.current[type]) return reasonsCache.current[type]!;
    const r = await api.get("/decision-reasons", {
      params: { type, active: "true" },
    });
    const items = (r.data.items ?? []) as ReasonOpt[];
    reasonsCache.current[type] = items;
    return items;
  }, []);
  ...
  // id passed from OtEntryRow itself — no inline arrow needed in JSX
  ...
  const toggleSelectAll = useCallback(() => {
    setSelectedIds((prev) =>
      prev.length === approvablePendingIds.length ? [] : approvablePendingIds,
    );
  }, [approvablePendingIds]);
  ...
  async function confirmBulkApprove() {
    ...
    if (!rowsToApprove.length) return toast.error("No valid rows to approve");
  ...
  async function confirmReject() {
    ...
    try {
      ...
      setDayItems((prev) =>
        prev.map((x) =>
          x._id === actingId ? { ...x, status: "REJECTED" } : x,
        ),
      );
      setRejectOpen(false);
      setActingId(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-7">
        {weekDates.map((d, i) => {
          const date = weekDateKeys[i]; // use pre-computed keys — no toYYYYMMDD per render
          return (
            <WeekDayCard
              key={date}
              date={d}
            ...
            <thead className="border-b border-gray-200/50 bg-gradient-to-r from-gray-50 to-white/80">
              <tr className="text-left">
                <th className="px-5 py-3.5 text-xs font-black uppercase tracking-wider text-gray-700">
                  {/* Uses memoized approvablePendingIds — no inline filter */}
                  <input
                    type="checkbox"
                    checked={
                      approvablePendingIds.length > 0 &&
                      selectedIds.length === approvablePendingIds.length
                    }
            ...
            <tbody className="divide-y divide-gray-100">
              {dayItems.map((it) => (
                <OtEntryRow
                  ...
                  isSelected={selectedSet.has(it._id)}
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-gray-600">
                      Adjust approved OT values if needed, especially for
                      NO_SHIFT or manually reviewed rows.
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-700">
                      Review:{" "}
                      {
                        bulkRows.filter((r) => !r.canApprove || r.warning)
                          .length
                      }
                    </div>
                  </div>
